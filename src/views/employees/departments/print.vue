...
<script>
import { getPersonalInfoByIdAPI, getJobInfoByIdAPI, getUserDetalByIdAPI } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getPersonalDetail() : this.getJobInfoById()
  },
  // 组件更新
  methods: {
    async getPersonalDetail() {
      this.formData = await getPersonalInfoByIdAPI(this.userId) // 获取个人基本信息
      console.log(this.formData)
    },
    async getJobInfoById() {
      const userInfo = await getUserDetalByIdAPI(this.userId)
      const jobInfo = await getJobInfoByIdAPI(this.userId) // 获取个人基本信息
